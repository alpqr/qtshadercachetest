// For a clean run, do rm -rf ~/.cache/qtshadercache before running or
// whatever is the equivalent on the platform.
...
public:
    ...
    void initializeGL() {
        ...
        QOpenGLShaderProgram *prog = m_programs[0];
...
{
//    qputenv("QT_LOGGING_RULES", "qt.opengl.diskcache=true");
    ...
    for (int i = 0; i < args.count(); ++i) {
        if (args[i] == QStringLiteral("--recompile"))
            DIFF = true;
        else if (args[i] == QStringLiteral("--count"))
            COUNT = args[++i].toInt();
    }

    qDebug("%d shaders", COUNT);
